...
export function FieldSwitchOrEdit({ label, hint, options, value, onChange, allowCustomValue = true }) {
  options = options || []
  const { setHint } = useContext(HintContext)
  const [isEditing, setIsEditing] = useState(false)
  const [customValue, setCustomValue] = useState(value)
  
  // Find if the current value is in the options
  const idx = options.findIndex(o => o.value === value)
  const selected = options[idx]
  
  // Handle direct value editing
  useEffect(() => {
    if (!isEditing && value !== customValue) {
  ...
  const prev = () => {
    ...
    if (nextIdx < 0) nextIdx = options.length - 1
    onChange(options[nextIdx].value)
    setCustomValue(options[nextIdx].value)
  }
    ...
    <div
      ...
      css={css`
        ...
        .fieldswitch-text {
          font-size: 0.9375rem;
          line-height: 1;
          cursor: ${allowCustomValue ? 'text' : 'default'};
          &:hover {
            text-decoration: ${allowCustomValue ? 'underline' : 'none'};
