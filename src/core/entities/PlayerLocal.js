    ...
    if (this.data.effect?.emote) {
      ...
      if (this.targetLockOn && this.lockedTarget) {
        ...
        if (Math.abs(rightDot) > Math.abs(forwardDot)) {
          ...
          if (rightDot > 0) {
            // Strafing right
            emote = this.running ? Emotes.RUN_STRAFE_RIGHT : Emotes.STRAFE_RIGHT
          } else {
            // Strafing left
            emote = this.running ? Emotes.RUN_STRAFE_LEFT : Emotes.STRAFE_LEFT
          }
        } else if (forwardDot < -0.5) {
          // Moving backward
          emote = this.running ? Emotes.RUN_BACKWARD : Emotes.WALK_BACKWARD
        } else {
