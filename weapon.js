...
if (world.isServer) {
  const overlapLayerMask = world.createLayerMask('environment', 'player')
  ...
  app.on('attack', data => {
    ...
    console.log(hits.map(h => `tag:${h.tag}|playerId:${h.playerId}`))
    ...
    for (const hit of hits) {
      ...
      // Handle mob hits - look for entities with MOB tag
      else if (hit.tag.startsWith('MOB')) {
        console.log('hit.tag', hit.tag)
        const [prefix, entityId] = hit.tag.split(':')
        if (!entityId) continue

        // Calculate damage
        let amount = num(MIN_DMG, MAX_DMG)
        let crit = num(0, 1, 1) < CRIT_CHANCE
        if (crit) amount *= CRIT_MULTIPLIER

        // Emit global damage event for PvP Core to handle
        app.emit('pvpcore:damage', {
          entityId: entityId,
          amount: amount,
          attackerId: playerId,
          crit: crit,
        ...
        // Send damage event for UI effects
        app.send('dmg', { entityId: entityId, amount, crit })
      }
    }
  })
